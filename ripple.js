...
let initPuzzle, puzzle, lastVert, lastHorz, moveHistory;
let boardStates, demoStepNum, puzzlePolyoList, largestPolyo;
...
function updateTextFields() {
  ...
  if (assistState == 0) { 
    if (errorCount && incompleteCount) {
      updateHTML('errortext', "there are errors and incomplete polyominos");
    } else if (errorCount) {
      updateHTML('errortext', "there are errors");
    } else if (incompleteCount) {
      updateHTML('errortext', "there are incomplete polyominos");
    } else {
      updateHTML('errortext', "there are no errors nor incomplete polyominos");
    }
  } else {
    updateHTML('errortext', "there are " + errorCount + " errors and " +
                            incompleteCount + " incomplete polyominos");
  }
...
function initStructures(puzzle) {
  ...
  // find all the polyominos
  puzzlePolyoList = findPolyominos();
  largestPolyo = 0;
  for (let i=0;i<puzzlePolyoList.length;i++) {
    if (puzzlePolyoList[i].length > largestPolyo) {
      largestPolyo = puzzlePolyoList[i].length;
    }
  }
  updateBoardStatus();
  drawBoard();
...
function updateBoardStatus() {
  ...
  //  1) duplicates of a digit in a polyomino
  //  2) too large of a value in a polyomino
  //  3) two digits too close to each other
  // also count the incompletes, which are polyominos
  // that don't have all of the digits filled correctly
  let errorCells = new Array();
  let incompletePolyos = new Array();
  incompleteCount = 0;

  for (let p=0;p<puzzlePolyoList.length;p++) {
    let polyo = puzzlePolyoList[p];
    let vcounts = new Array();
    for (let i=1;i<=polyo.length;i++) {
      vcounts[i] = 0;
    }
    for (let i=0;i<polyo.length;i++) {
      let pcell = polyo[i].split(",");
      let py = pcell[0];
      let px = pcell[1];
      let pv = globalBoardValues[py][px];
      if (pv > polyo.length && (errorCells.indexOf(pcell) == -1)) {
        errorCells.push(polyo[i]);
      }
      vcounts[pv]++;
    }
    for (let c=1;c<=polyo.length;c++) {
      // if more than one, mark all in the polyo with that number in error
      if (vcounts[c] > 1) {
        for (let i=0;i<polyo.length;i++) {
          let pcell = polyo[i].split(",");
          let py = pcell[0];
      ...
      // if zero, then this is an incomplete polyo
      } else if (vcounts[c] == 0) {
        if (incompletePolyos.indexOf(p) == -1) {
          incompletePolyos.push(p);
        }
      }
    }
  }
  ...
  // in assistState 2, color completed polyominos light green,
  ...
  for (let p=0;p<puzzlePolyoList.length;p++) {
    ...
    for (let i=0;i<polyo.length;i++) {
      ...
      let py = pcell[0];
      let px = pcell[1];
      if ((assistState == 2) && (incompletePolyos.indexOf(p) == -1)) {
        globalBoardColors[py][px] = completeFillColor;
      } else {
        globalBoardColors[py][px] = emptyCellColor;
      }
      if ((assistState == 2) && (errorCells.indexOf(polyo[i]) != -1)) {
  ...
  incompleteCount = incompletePolyos.length;
  updateTextFields();
